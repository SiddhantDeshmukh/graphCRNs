...
# Multi-layer GCN Regressor
class GCNReg(tf.keras.Model):
  def __init__(self, *args, **kwargs):
    super().__init__(*args, **kwargs)
    self.gcn0 = tfg.layers.GCN(16, activation=tf.nn.relu)
    # self.gcn1 = tfg.layers.GCN()
    self.linear1 = tf.keras.layers.Dense(
        8, activation='relu')  # strictly positive
  ...
  def call(self, inputs, training=None, mask=None, cache=None):
    ...
    h = self.gcn0([h, edge_index, edge_weight], cache=cache)
    h = self.dropout(h, training=training)
    h = self.linear1([h, edge_index, edge_weight], cache=cache)
    return h
...
for _ in tqdm(range(num_test_iterations)):
  logits = forward(graph)
end_time = time.time()
print("mean forward time: {} seconds".format(
    (end_time - start_time) / num_test_iterations))
